...
import model.*;
import model.thongke.uMatKhauMoi;
...
public class BeanNguoiDung implements Serializable {
    ...
    private uMatKhauMoi matKhauMoi;
    ...
    public BeanNguoiDung() {
        ...
        matKhauMoi = new uMatKhauMoi();
    }

    /* 
        Update : Sử dụng function get session ;
     */
    public void dangNhap() {
        ...
        if (tk != null) {
            ...
            if (taiKhoanDangNhap.isIsAdmin()) {
                // Đăng nhập nếu là admin thì chuyển sang trang quản trị
                ExternalContext ec = FacesContext.getCurrentInstance().getExternalContext();
                try {
                    ec.redirect(ec.getRequestContextPath() + "/faces/Admin/adminTongQuan.xhtml");
                } catch (IOException e) {
                }
            }
        } else {
    ...
    // Update : Sử dụng model mật khẩu mới
    // Return :  Thêm tài khoản vào danh sách
    public void dangKy() {
        if (taiKhoanDangNhap.getTenTaiKhoan().isEmpty() || matKhauMoi.getMatKhauMoi().isEmpty()) {
        ...
        taiKhoanDangNhap.setMatKhau(matKhauMoi.getMatKhauMoi());
    ...
    public void caNhan() {
        HttpSession session = getSession();
        if ((TaiKhoan) session.getAttribute("TaiKhoan") == null) {
    ...
    // Update : Cập nhật thông tin riêng , đổi mật khẩu riêng
    // Return :  
    //          Cập nhật thành công :  thay đổi session 
    //          Cập nhật thất bại   :  lấy lại thông tin đăng nhập cũ 
    public void capNhatThongTin() {
        TaiKhoan taiKhoanDangNhapCu = new TaiKhoan((TaiKhoan) getSession().getAttribute("TaiKhoan"));

        if (dao.DAOTaiKhoan.update(taiKhoanDangNhap)) {
            msg.Message.addMessage("Thành Công", "Cập nhật thành công!");
            getSession().setAttribute("TaiKhoan", taiKhoanDangNhap);
        } else {
            taiKhoanDangNhap = taiKhoanDangNhapCu;
    ...
    public void doiMatKhau() throws IOException {
        ...
        if (!matKhauMoi.getMatKhau().equals(taiKhoanDangNhapCu.getMatKhau())) {
            msg.Message.errorMessage("Thất Bại", "Mật khẩu không đúng !");
            return;
        }
        taiKhoanDangNhap.setMatKhau(matKhauMoi.getMatKhauMoi());
        if (!dao.DAOTaiKhoan.update(taiKhoanDangNhap)) {
            ...
            getSession().setAttribute("TaiKhoan", tk);
            FacesContext.getCurrentInstance().getExternalContext().redirect("caNhan.xhtml");
        }

    }

    public String lichSu() {
        TaiKhoan tk = (TaiKhoan) getSession().getAttribute("TaiKhoan");
    ...
    public uMatKhauMoi getMatKhauMoi() {
        return matKhauMoi;
    }

    public void setMatKhauMoi(uMatKhauMoi matKhauMoi) {
